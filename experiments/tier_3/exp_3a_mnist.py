...
Data augmentation:
  ...
  - RandomCrop(28, padding=4), NO horizontal flip (digits not flip-invariant)
...
def run_experiment(results_dir: str):
    """Run MNIST classification. Returns (metrics, arch, config_dict)."""
    device = get_device()
    ...
    # MNIST: .data is torch.Tensor [N, 28, 28] uint8, .targets is torch.Tensor [N]
    # Convert to [N, 28, 28, 1] uint8 HWC numpy for PIL-based augmentation
    ...
    # MNIST: grayscale, no AutoAugment (requires RGB), no flip (digits not flip-invariant)
    batch_size = 256
    loaders = create_image_augmented_loaders(
        ...
        affine_degrees=15,         # ±15° rotation (was ±10°)
        # ElasticTransform moved to GPU (elastic_enabled in config below)
    )
    ...
    config = ASANNConfig(
        ...
        c_stem_init=32,               # Stronger initial capacity (was 8)
        spatial_downsample_stages=0,   # No stride-2 downsampling (28×28 throughout, GAP at end)
        max_channels=512,
        initial_num_layers=2,          # Start with 2 conv layers (was 1)
        surgery_interval_init=400,
        warmup_steps=1200,
        complexity_target_auto=True,
        layer_add_cooldown_steps=3000, # Faster growth (was 5000)
        ...
        stability_healthy_epochs=6,    # Slightly faster stability (was 8)
        recovery_epochs=3,
        # Augmentation tuning
        cutout_size=2,                # 28→26 (was 14→too aggressive)
        mixup_alpha=0.1,              # Reduced from 0.2 (MNIST is simple)
        # GPU elastic deformation (replaces CPU ElasticTransform)
        elastic_enabled=True,
        elastic_alpha=30.0,           # Handwriting-like deformation
        elastic_sigma=4.0,            # Smooth displacement field
        # Feature toggles
        op_gating_enabled=False,  # Pure op(x), no gated residual
        spinal_enabled=False,     # Standard ASANN (no spinal)
        # LR tuning: lower base LR + working dead zone + tighter scale
        optimizer=ASANNOptimizerConfig(
            base_lr=1e-3,                          # Default LR (was 5e-4)
            lr_controller_scale_max=3.0,           # Max 3x base LR
            lr_controller_dead_zone=(-0.03, 0.03), # Narrow: lets real signals through
        ),
        device=device,
    )
    ...
    # ===== 8. Evaluate on test set (use best saved model) =====
...
if __name__ == "__main__":
    ...
    name, metrics, arch, elapsed, status = run_classification_experiment_wrapper(
        "MNIST", run_experiment, tier_results
    )
    if status != "OK":
