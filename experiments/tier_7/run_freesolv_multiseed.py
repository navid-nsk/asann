"""Run FreeSolv scaffold experiment with multiple seeds and aggregate results."""
import subprocess
import sys
import os
import json
...
exp_script = proj_root / "experiments" / "tier_7" / "exp_7l_moleculenet_freesolv.py"

all_results = {}

for seed in SEEDS:
    print(f"\n{'='*70}")
    print(f"  FreeSolv {SPLIT} split -- seed {seed}")
    print(f"{'='*70}\n")

    env = os.environ.copy()
    env["FREESOLV_SPLIT"] = SPLIT
    env["FREESOLV_SEED"] = str(seed)

    # Clean old results for this seed
    results_dir = results_base / f"moleculenet_freesolv_{SPLIT}_s{seed}"
...
if all_results:
    ...
    # Save aggregate
    agg_path = results_base / f"freesolv_{SPLIT}_multiseed_summary.json"
    agg = {
