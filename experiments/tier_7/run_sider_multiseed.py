"""Run SIDER scaffold experiment with multiple seeds and aggregate results."""
import subprocess
import sys
import os
import json
...
exp_script = proj_root / "experiments" / "tier_7" / "exp_7j_moleculenet_sider.py"

all_results = {}

for seed in SEEDS:
    print(f"\n{'='*70}")
    print(f"  SIDER {SPLIT} split -- seed {seed}")
    print(f"{'='*70}\n")

    env = os.environ.copy()
    env["SIDER_SPLIT"] = SPLIT
    env["SIDER_SEED"] = str(seed)

    # Clean old results for this seed
    results_dir = results_base / f"moleculenet_sider_{SPLIT}_s{seed}"
...
if all_results:
    ...
    print(f"\n  Correct AUROC (global, {all_results[list(all_results.keys())[0]].get('n_tasks', 27)}-task):")
    ...
    # Save aggregate
    agg_path = results_base / f"sider_{SPLIT}_multiseed_summary.json"
    agg = {
