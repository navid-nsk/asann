"""
Experiment 7G: MoleculeNet ESOL -- Aqueous Solubility Prediction
================================================================
Dataset: ESOL (Delaney) via MoleculeNet S3 / PyTDC
Samples: ~1,128 molecules
Features: 524 (512-bit Morgan fingerprints + 12 RDKit descriptors)
Target: log solubility in mol/L (continuous, regression)
Task: Predict aqueous solubility from molecular structure.
...
def run_experiment(results_dir: str):
    """Run MoleculeNet ESOL solubility regression experiment."""
    try:
        from tier_7.bio_utils import load_moleculenet_esol, smiles_to_molecular_graphs
    except ImportError:
    ...
    seed = int(os.environ.get("ESOL_SEED", "42"))
    ...
    SPLIT_MODE = os.environ.get("ESOL_SPLIT", "scaffold")
    print(f"  Split mode: {SPLIT_MODE}")

    # ===== 1. Load dataset with SMILES =====
    print("  Loading ESOL dataset...")
    X, y, smiles_list, valid_indices = load_moleculenet_esol(return_smiles=True)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ("scaffold", "random"):
        os.environ["ESOL_SPLIT"] = sys.argv[1]

    split_mode = os.environ.get("ESOL_SPLIT", "scaffold")
    esol_seed = os.environ.get("ESOL_SEED", "42")
    exp_dir_name = f"moleculenet_esol_{split_mode}_s{esol_seed}"
