"""Run Lipophilicity scaffold experiment with multiple seeds and aggregate results."""
import subprocess
import sys
import os
import json
...
exp_script = proj_root / "experiments" / "tier_7" / "exp_7m_moleculenet_lipo.py"

all_results = {}

for seed in SEEDS:
    print(f"\n{'='*70}")
    print(f"  Lipophilicity {SPLIT} split -- seed {seed}")
    print(f"{'='*70}\n")

    env = os.environ.copy()
    env["LIPO_SPLIT"] = SPLIT
    env["LIPO_SEED"] = str(seed)

    # Clean old results for this seed
    results_dir = results_base / f"moleculenet_lipo_{SPLIT}_s{seed}"
...
if all_results:
    ...
    # Save aggregate
    agg_path = results_base / f"lipo_{SPLIT}_multiseed_summary.json"
    agg = {
