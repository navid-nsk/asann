"""
Experiment 7M: MoleculeNet Lipophilicity -- logD Prediction
============================================================
Dataset: Lipophilicity from ChEMBL
Samples: ~4,200 molecules
Features: 524 (512-bit Morgan fingerprints + 12 RDKit descriptors)
Target: Experimental lipophilicity (logD at pH 7.4, continuous, regression)
Task: Predict lipophilicity from molecular structure.
...
def run_experiment(results_dir: str):
    """Run MoleculeNet Lipophilicity regression experiment."""
    try:
        from tier_7.bio_utils import load_lipophilicity_csv, smiles_to_molecular_graphs
    except ImportError:
    ...
    seed = int(os.environ.get("LIPO_SEED", "42"))
    ...
    SPLIT_MODE = os.environ.get("LIPO_SPLIT", "scaffold")
    print(f"  Split mode: {SPLIT_MODE}")

    # ===== 1. Load dataset from CSV =====
    csv_path = os.path.join(_proj_root, "data", "Lipophilicity", "Lipophilicity.csv")
    if not os.path.exists(csv_path):
        raise RuntimeError(f"Lipophilicity CSV not found at {csv_path}")

    print("  Loading Lipophilicity dataset...")
    X, y, smiles_list, valid_indices = load_lipophilicity_csv(csv_path, return_smiles=True)
    ...
    config = ASANNConfig(
        ...
        complexity_target=80000.0,     # Slightly higher for larger dataset
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ("scaffold", "random"):
        os.environ["LIPO_SPLIT"] = sys.argv[1]

    split_mode = os.environ.get("LIPO_SPLIT", "scaffold")
    lipo_seed = os.environ.get("LIPO_SEED", "42")
    exp_dir_name = f"moleculenet_lipo_{split_mode}_s{lipo_seed}"
