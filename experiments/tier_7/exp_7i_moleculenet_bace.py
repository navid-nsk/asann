"""
Experiment 7I: MoleculeNet BACE -- Beta-secretase 1 (BACE-1) Inhibitors
========================================================================
Dataset: BACE from our own bace.csv
Samples: ~1,513 molecules
Features: 2048 (Morgan fingerprints from SMILES)
Classes: 2 (inhibitor=1, non-inhibitor=0)
Task: Binary classification -- predict BACE-1 inhibition.

Balanced dataset (~46% positive). Uses CrossEntropyLoss with class weights.
Supports scaffold and random splitting.
In scaffold mode, reports both KA-GNN-style and FedLG-style buggy evaluations.

Usage:
  python exp_7i_moleculenet_bace.py [scaffold|random]
  BACE_SPLIT=random python exp_7i_moleculenet_bace.py
...
def run_experiment(results_dir: str):
    """Run MoleculeNet BACE classification experiment."""
    try:
        from tier_7.bio_utils import load_bace_csv, smiles_to_molecular_graphs
    except ImportError:
    ...
    seed = int(os.environ.get("BACE_SEED", "137"))
    ...
    # ===== 1. Load dataset from our own bace.csv =====
    csv_path = os.path.join(_proj_root, "data", "bace", "bace.csv")
    if not os.path.exists(csv_path):
        raise RuntimeError(f"BACE CSV not found at {csv_path}")

    print("  Loading BACE dataset from bace.csv...")
    X, y, n_classes, smiles_list, valid_indices = load_bace_csv(
        csv_path, return_smiles=True
    )
    print(f"  Loaded: {X.shape[0]} samples, {X.shape[1]} features, {n_classes} classes")

    # ===== 2. Split =====
    SPLIT_MODE = os.environ.get("BACE_SPLIT", "scaffold")
    ...
    if SPLIT_MODE == "scaffold":
        ...
        # --- KA-GNN style buggy eval (dual model) ---
        buggy_max_best, buggy_mean_best = buggy_auroc(
            y_test_np, probs, batch_size=128, n_trials=501
        )
        print(f"  AUROC KA-GNN buggy (best model): max={buggy_max_best:.4f}")
        ...
        print(f"  AUROC KA-GNN buggy (final model): max={buggy_max_final:.4f} "
              f"(correct={final_correct:.4f})")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ("scaffold", "random"):
        os.environ["BACE_SPLIT"] = sys.argv[1]

    split_mode = os.environ.get("BACE_SPLIT", "scaffold")
    exp_name = f"MoleculeNet BACE ({split_mode} split)"
    print(f"\n{'='*60}")
    print(f"  {exp_name}")
    print(f"{'='*60}\n")

    bace_seed = os.environ.get("BACE_SEED", "137")
    exp_dir_name = f"moleculenet_bace_{split_mode}_s{bace_seed}"
