"""
Experiment 7K: MoleculeNet Tox21 -- Toxicity (12-task)
======================================================
Dataset: Tox21 from our own tox21.csv
Samples: ~7,831 molecules
Features: 2048 (Morgan fingerprints from SMILES)
Tasks: 12 binary classification targets (NR-AR, NR-AhR, SR-ARE, etc.)
       Each task has missing labels (NaN).
...
Usage:
  python exp_7k_moleculenet_tox21.py [scaffold|random]
  TOX21_SPLIT=random python exp_7k_moleculenet_tox21.py
...
def run_experiment(results_dir: str):
    """Run MoleculeNet Tox21 12-task multi-label experiment."""
    try:
        from tier_7.bio_utils import load_tox21_csv, smiles_to_molecular_graphs
    except ImportError:
    ...
    seed = int(os.environ.get("TOX21_SEED", "137"))
    ...
    # ===== 1. Load dataset =====
    csv_path = os.path.join(_proj_root, "data", "tox21", "tox21.csv")
    if not os.path.exists(csv_path):
        raise RuntimeError(f"Tox21 CSV not found at {csv_path}")

    print("  Loading Tox21 dataset from tox21.csv...")
    X, y_multi, task_names, smiles_list, valid_indices = load_tox21_csv(csv_path)
    n_tasks = len(task_names)
    print(f"  Loaded: {X.shape[0]} molecules, {X.shape[1]} features, {n_tasks} tasks")

    # ===== 2. Split =====
    SPLIT_MODE = os.environ.get("TOX21_SPLIT", "scaffold")
    ...
    print(f"\n  === Tox21 Results ({SPLIT_MODE} split, {n_tasks}-task) ===")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ("scaffold", "random"):
        os.environ["TOX21_SPLIT"] = sys.argv[1]

    split_mode = os.environ.get("TOX21_SPLIT", "scaffold")
    exp_name = f"MoleculeNet Tox21 ({split_mode} split)"
    print(f"\n{'='*60}")
    print(f"  {exp_name}")
    print(f"{'='*60}\n")

    tox21_seed = os.environ.get("TOX21_SEED", "137")
    exp_dir_name = f"moleculenet_tox21_{split_mode}_s{tox21_seed}"
