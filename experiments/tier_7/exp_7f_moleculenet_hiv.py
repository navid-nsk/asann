"""
Experiment 7F: MoleculeNet HIV -- HIV Replication Inhibition
============================================================
Dataset: HIV from our own hiv.tab (PyTDC source)
Samples: ~41,127 molecules
Features: 2048 (Morgan fingerprints from SMILES)
Classes: 2 (active=1, inactive=0)
Task: Binary classification -- predict whether a compound inhibits HIV replication.

Highly imbalanced (~3% positive). Uses focal loss with capped class weights.
Baseline AUROC ~0.76-0.82 (random forest / GCN).
...
41K samples at batch_size=256 gives ~113 steps/epoch.

Usage:
  python exp_7f_moleculenet_hiv.py [scaffold|random]
  HIV_SPLIT=random python exp_7f_moleculenet_hiv.py
...
def run_experiment(results_dir: str):
    """Run MoleculeNet HIV classification experiment."""
    # Check dependencies
    try:
        from tier_7.bio_utils import (
            load_hiv_tab, smiles_to_molecular_graphs,
        )
    except ImportError:
    ...
    seed = int(os.environ.get("HIV_SEED", "137"))
    ...
    # ===== 1. Load dataset from our own hiv.tab =====
    tab_path = os.path.join(_proj_root, "data", "hiv.tab")
    if not os.path.exists(tab_path):
        raise RuntimeError(f"HIV tab not found at {tab_path}")

    print("  Loading HIV dataset from hiv.tab...")
    X, y, n_classes, smiles_list, valid_indices = load_hiv_tab(
        tab_path, return_smiles=True
    )
    print(f"  Loaded: {X.shape[0]} samples, {X.shape[1]} features, {n_classes} classes")

    # ===== 2. Split =====
    SPLIT_MODE = os.environ.get("HIV_SPLIT", "scaffold")
    ...
    config = ASANNConfig(
        ...
        # Overfitting thresholds: extreme imbalance causes large gaps.
    ...
    # ===== 6. Focal loss with capped class weights =====
    ...
    class_weights = np.clip(class_weights, None, class_weights.min() * max_weight_ratio)
    class_weights = torch.tensor(class_weights, dtype=torch.float32).to(device)
    print(f"  Class weights (capped {max_weight_ratio}x): {class_weights.cpu().tolist()}")
    ...
    # AUROC / AUPRC -- correct evaluation (all test samples)
    eval_model.eval()
    with torch.no_grad():
    ...
    try:
        best_auroc = float(roc_auc_score(y_test_np, probs))
    except ValueError:
        best_auroc = 0.5
    try:
        best_auprc = float(average_precision_score(y_test_np, probs))
    except ValueError:
        best_auprc = 0.0

    metrics["auroc"] = best_auroc
    metrics["auprc"] = best_auprc
    metrics["split_mode"] = SPLIT_MODE
    metrics["n_train"] = n_train
    metrics["n_val"] = n_val
    metrics["n_test"] = n_test
    print(f"  AUROC (correct, all {len(y_test_np)} samples): {best_auroc:.4f}")
    ...
    if SPLIT_MODE == "scaffold":
        ...
        fedlg_best, _ = fedlg_buggy_auroc(y_test_np, probs, batch_size=128, n_epochs=501)
        fedlg_final, _ = fedlg_buggy_auroc(y_test_np, probs_final, batch_size=128, n_epochs=501)
        ...
        print(f"  Inflation KA-GNN: {buggy_max - best_auroc:.4f}, "
              f"FedLG: {fedlg_max - best_auroc:.4f}")
...
if __name__ == "__main__":
    ...
    # Support command-line split mode: python exp_7f_moleculenet_hiv.py [scaffold|random]
    if len(sys.argv) > 1 and sys.argv[1] in ("scaffold", "random"):
        os.environ["HIV_SPLIT"] = sys.argv[1]

    split_mode = os.environ.get("HIV_SPLIT", "scaffold")
    exp_name = f"MoleculeNet HIV ({split_mode} split)"
    print(f"\n{'='*60}")
    print(f"  {exp_name}")
    print(f"{'='*60}\n")

    # Use split-specific and seed-specific results directory
    hiv_seed = os.environ.get("HIV_SEED", "137")
    exp_dir_name = f"moleculenet_hiv_{split_mode}_s{hiv_seed}"
