"""
Experiment 5d: Kuramoto-Sivashinsky (PINNacle Benchmark)
=========================================================
Dataset: PINNacle Kuramoto_Sivashinsky.dat
Samples: ~65k (subsampled to 50k) | Input: (x, t) → 2 features | Output: u → 1 target
PDE: u_t + u·u_x + u_xx + u_xxxx = 0 — Chaotic, 4th-order PDE.

This is the hardest 1D PDE: chaotic dynamics with 4th-order spatial derivatives.
ASANN should discover: high-order derivative ops + polynomial (u²) terms.
"""
...
def run_experiment(results_dir: str):
    """Run Kuramoto-Sivashinsky experiment. Returns (metrics, arch, config_dict, train_metrics)."""
    device = get_device()

    # ===== 1. Load dataset =====
    print("  Loading PINNacle Kuramoto-Sivashinsky dataset...")
    X, y = load_pinnacle_data("kuramoto_sivashinsky", max_samples=50000)
    ...
    config = ASANNConfig(
        ...
        # More surgery budget for chaotic dynamics
...
if __name__ == "__main__":
    ...
    name, metrics, arch, elapsed, status = run_experiment_wrapper(
        "Kuramoto-Sivashinsky", run_experiment, tier_results
    )
    if status != "OK":
