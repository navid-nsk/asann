"""
Experiment 5b: Burgers 1D (PINNacle Benchmark)
================================================
Dataset: PINNacle burgers1d.dat (COMSOL time-encoded)
Samples: 1,111 | Input: (x, t) → 2 features | Output: u → 1 target
PDE: u_t + u·u_x = ν·u_xx — Nonlinear advection-diffusion with shock formation.

ASANN should discover: derivative ops + polynomial (u²) terms.
"""
...
def run_experiment(results_dir: str):
    """Run Burgers 1D experiment. Returns (metrics, arch, config_dict, train_metrics)."""
    device = get_device()

    # ===== 1. Load dataset =====
    print("  Loading PINNacle Burgers 1D dataset...")
    X, y = load_pinnacle_data("burgers1d")
...
if __name__ == "__main__":
    ...
    name, metrics, arch, elapsed, status = run_experiment_wrapper(
        "Burgers 1D", run_experiment, tier_results
    )
    if status != "OK":
