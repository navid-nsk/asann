"""
Experiment 6f: PEMS-BAY Traffic Forecasting
=============================================
Dataset: PEMS-BAY (San Francisco Bay Area traffic speed sensors)
Sensors: 325 | Time-series regression
...
Each forward pass receives a full graph snapshot: [325, window] -> [325, horizon].
...
def run_experiment(results_dir: str):
    """Run PEMS-BAY traffic forecasting experiment."""
    # Reproducibility
    SEED = 42
    random.seed(SEED)
    np.random.seed(SEED)
    ...
    X_all, Y_all, adj = load_traffic_preprocessed("pems-bay", window=window, horizon=horizon)
...
if __name__ == "__main__":
    ...
    name, metrics, arch, elapsed, status = run_experiment_wrapper(
        "PEMS-BAY", run_experiment, tier_results
    )
    if status != "OK":
