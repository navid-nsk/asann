"""
Experiment 6b: PubMed Node Classification
==========================================
Dataset: Planetoid/PubMed (citation graph)
Nodes: 19,717 | Edges: 88,648 | Features: 500 | Classes: 3
...
def run_experiment(results_dir: str):
    """Run PubMed node classification experiment."""
    # Reproducibility
    SEED = 42
    random.seed(SEED)
    np.random.seed(SEED)
    ...
    data, num_classes = load_pyg_citation("PubMed")
    ...
    # split (60 train / 500 val / 1000 test). The standard split has only
    # 60 labeled nodes which severely limits achievable accuracy.
    N = data.num_nodes
    ...
    # Input feature dropout — reduced importance with 60% train split
    # (0.5 was critical for semi-supervised with only 60 labeled nodes)
...
if __name__ == "__main__":
    ...
    name, metrics, arch, elapsed, status = run_experiment_wrapper(
        "PubMed", run_experiment, tier_results
    )
    if status != "OK":
